...
const Curtain = ({
  ...
  useEffect(() => {
    console.log('curtain 2 isPresent changed...', `isPresent: ${isPresent}`);
  ...
  const renderBlock = (i: number, j: number): JSX.Element => {
    ...
    // console.log('block', blockVariants);
  ...
  console.log('<Curtain2 />', curtainState, curtainVariants);

  return (
    isMounted &&
    createPortal(
      <CurtainOverlay finished={!curtainState}>
        <CurtainWrapper
